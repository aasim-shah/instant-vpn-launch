...
flowchart TD
    ...
    Q1 -- Streaming --> S["smartConnectForStreaming()"]
    Q1 -- P2P --> P["smartConnectForP2P()"]
    Q1 -- Gaming --> L["strategy = LOWEST_LATENCY"]
    Q1 -- User picked country --> C["criteria = SmartConnectCriteria(country)"]
...
sdk.<span className="fn">smartConnect</span>(strategy = <span className="type">SelectionStrategy</span>.LOWEST_LOAD)</code></pre>
    ...
    criteria = <span className="type">SmartConnectCriteria</span>({'\n'}
        ...
        preferredRegion = <span className="str">"Virginia"</span>,{'\n'}
...
sdk.<span className="fn">smartConnectByRegion</span>(<span className="str">"Virginia"</span>)</code></pre>
    ...
    <span className="kw">val</span> server: <span className="type">Server</span>,              <span className="cmt">// The selected server</span>{'\n'}
    ...
    <span className="kw">val</span> alternates: <span className="type">List&lt;Server&gt;</span>    <span className="cmt">// Next best options</span>{'\n'}
)</code></pre>

        <h3>Using Alternates</h3>
        <pre><code><span className="kw">val</span> result = sdk.<span className="fn">smartConnect</span>(){'\n'}
{'\n'}
<span className="kw">if</span> (result <span className="kw">is</span> <span className="type">VpnResult.Success</span>) {'{'}{'\n'}
    ...
    <span className="kw">val</span> fallback = result.data.alternates.<span className="fn">firstOrNull</span>(){'\n'}
{'\n'}
    <span className="cmt">// If primary fails, try fallback</span>{'\n'}
