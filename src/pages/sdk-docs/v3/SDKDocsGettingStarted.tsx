  ...
  return (
    <>
      <SEO 
        ...
        canonical="/sdk/docs/v3/getting-started"
      />
...
{'}'}</code></pre>
    ...
    <span className="fn">implementation</span>(<span className="str">"com.fyreway.vpn:fyreway-vpn-sdk:1.2.0"</span>){'\n'}
{'\n'}
...
{'\n'}
    ...
    &lt;<span className="kw">uses-permission</span> android:name=<span className="str">"android.permission.ACCESS_WIFI_STATE"</span> /&gt;{'\n'}
{'\n'}
...
&lt;/<span className="kw">manifest</span>&gt;</code></pre>
        ...
        <p>Initialize <strong>once</strong> in your <code>Application.onCreate()</code> passing the Android <code>Context</code>. Never initialize in an Activity or Fragment.</p>
...
{'\n'}
        <span className="type">FyreWaySDK</span>.<span className="fn">initialize</span>(<span className="kw">this</span>, config);{'\n'}
    {'}'}{'\n'}
{'}'}</code></pre>
        ...
        <h3>Production Security Configuration</h3>
        <p>
          In production builds, the SDK enforces a Layer 0 authentication gate. You must provide <code>sdkAuthMode</code> and <code>customerId</code>
          (plus the mode-specific secret). If these are missing, <code>FyreWayConfig.build()</code> throws at startup.
          See the <a href="/sdk/docs/v3/configuration#security">Configuration Reference</a> for all three modes (TOTP, CHALLENGE, PLAY_INTEGRITY).
        </p>
        <div className="sdk-code-header"><span>Kotlin — production configuration (TOTP)</span></div>
        <pre><code><span className="type">FyreWaySDK</span>.<span className="fn">initialize</span>(<span className="kw">this</span>) {'{'}{'\n'}
    baseUrl = <span className="type">BuildConfig</span>.API_BASE_URL{'\n'}
{'\n'}
    <span className="cmt">// Layer 0 — required in non-debug builds</span>{'\n'}
...
{'\n'}
    <span className="cmt">// Optional: certificate pinning</span>{'\n'}
...
{'}'}</code></pre>
        ...
        <PageNav 
          prev={{ href: '/sdk/docs/v3/whats-new', title: "What's New" }}
          next={{ href: '/sdk/docs/v3/server-discovery', title: 'Server Discovery' }} 
        />
      </SDKDocsLayout>
    </>
  );
