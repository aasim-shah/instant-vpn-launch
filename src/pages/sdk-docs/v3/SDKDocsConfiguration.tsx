  ...
  return (
    <>
      <SEO 
        ...
        canonical="/sdk/docs/v3/configuration"
      />
      <SDKDocsLayout title="Configuration">
        ...
        <h2>FyreWayConfig</h2>
        <p>
          Pass to <code>FyreWaySDK.initialize()</code>. <code>baseUrl</code> is always required.
          In production builds, <code>sdkAuthMode</code> and <code>customerId</code> (plus the mode-specific secret) are also required —
          <code>FyreWayConfig.build()</code> throws at startup if they are missing. See the Security section below.
        </p>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                Requires Android <code>Context</code> at SDK initialization time, the Play Integrity dependency, and Google Play distribution.
              </td>
            </tr>
          </tbody>
        </table>
...
{' }'}</code></pre>
        <blockquote>
          <strong>Security:</strong> Never hardcode TOTP seeds or HMAC secrets in source control. Use <code>BuildConfig</code> fields injected at build time from your CI/CD secrets store.
        </blockquote>
...
-dontwarn okio.**</code></pre>
        ...
        <PageNav 
          prev={{ href: '/sdk/docs/v3/ui-utilities', title: 'UI Utilities' }}
        />
      </SDKDocsLayout>
    </>
  );
