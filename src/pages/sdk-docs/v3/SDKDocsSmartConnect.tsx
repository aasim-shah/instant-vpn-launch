  ...
  return (
    <>
      <SEO 
        ...
        canonical="/sdk/docs/v3/smart-connect"
      />
...
sdk.<span className="fn">smartConnectForCategory</span>(category = <span className="str">"p2p"</span>,      tier = <span className="str">"premium"</span>, country = <span className="str">"NL"</span>)</code></pre>
    ...
    <span className="kw">val</span> score: <span className="type">Double</span>,                       <span className="cmt">// 0.0 – 1.0</span>{'\n'}
    <span className="kw">val</span> reason: <span className="type">String</span>,                      <span className="cmt">// Human-readable explanation</span>{'\n'}
    <span className="kw">val</span> alternatives: <span className="type">List&lt;Server&gt;</span>           <span className="cmt">// Next-best options (empty on offline path)</span>{'\n'}
)</code></pre>
        <p>If you need to display the strategy or criteria used in your UI, store those values in your own ViewModel state alongside the SDK result.</p>
...
{'}'}</code></pre>
        ...
        <p>For simple one-tap connection experiences, use <code>smartConnectByLocation()</code> — it picks the best server near the user with no configuration required:</p>
        <pre><code><span className="cmt">// One-tap: best server near the user</span>{'\n'}
<span className="kw">val</span> result = sdk.<span className="fn">smartConnectByLocation</span>(strategy = <span className="type">SelectionStrategy</span>.BALANCED){'\n'}
<span className="kw">if</span> (result <span className="kw">is</span> <span className="type">VpnResult.Success</span>) {'{'}{'\n'}
    <span className="kw">val</span> server = result.data.server{'\n'}
    <span className="fn">println</span>(<span className="str">"${'{'}server.flagEmoji{'}'} ${'{'}server.name{'}'}"</span>)  <span className="cmt">// "🇺🇸 New York, US"</span>{'\n'}
    <span className="cmt">// Pass server to createProfile() and establish tunnel</span>{'\n'}
{'}'}</code></pre>

        <PageNav 
          prev={{ href: '/sdk/docs/v3/connection-lifecycle', title: 'Connection Lifecycle' }}
          next={{ href: '/sdk/docs/v3/error-handling', title: 'Error Handling' }} 
        />
      </SDKDocsLayout>
    </>
  );
