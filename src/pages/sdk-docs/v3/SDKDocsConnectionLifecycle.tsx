  ...
  return (
    <>
      <SEO 
        ...
        canonical="/sdk/docs/v3/connection-lifecycle"
      />
...
{'}'}</code></pre>
        ...
        <p>
          The SDK also supports Amnezia profiles via <code>VpnProtocol.AMNEZIA</code>. Pass <code>protocol = "amnezia"</code> (or the registry constant from <code>getRegistryConstants()</code>) in your <code>CreateProfileRequest</code>. The SDK normalizes backend variants such as <code>"amneziawg-v20"</code> automatically and exposes the VPN config via <code>VpnProfile.vpnConfig.raw</code>.
        </p>
...
{'\n'}
<span className="cmt">// Report to backend — no args needed, device identified by token</span>{'\n'}
sdk.<span className="fn">reportDisconnected</span>()</code></pre>

        <hr />

        <h2>Profile Management</h2>
        <h3>List Stored Profiles</h3>
        <pre><code><span className="cmt">// Profiles created via createProfile() are stored locally</span>{'\n'}
<span className="kw">val</span> stored = sdk.<span className="fn">getStoredProfiles</span>(){'\n'}
stored.<span className="fn">forEach</span> {'{'} p -&gt; <span className="fn">println</span>(<span className="str">"${'{'}p.peerIdentifier{'}'} on ${'{'}p.serverName{'}'}"</span>) {'}'}</code></pre>

        <h3>Revoke a Profile</h3>
        <pre><code><span className="cmt">// Revoke via backend (requires serverId + clientName)</span>{'\n'}
sdk.<span className="fn">revokeClient</span>({'\n'}
    ...
    protocol = <span className="type">VpnProtocol</span>.WIREGUARD{'\n'}
        ...
        <pre><code><span className="cmt">// Get shareable config text from the local profile</span>{'\n'}
<span className="kw">val</span> configText = profile.<span className="fn">toVpnConfig</span>(){'\n'}
{'\n'}
<span className="cmt">// Or re-export from the backend</span>{'\n'}
...
file.<span className="fn">writeText</span>(configText)</code></pre>
        ...
        <pre><code><span className="cmt">// Observe state changes reactively (collect in your ViewModel)</span>{'\n'}
<span className="kw">val</span> stateFlow = sdk.connectionState{'\n'}
{'\n'}
stateFlow.<span className="fn">collect</span> {'{'} state -&gt;{'\n'}
...
{'}'}{'\n'}
{'\n'}
<span className="cmt">// Or snapshot the active connection directly</span>{'\n'}
<span className="kw">val</span> active = sdk.<span className="fn">getActiveConnection</span>(){'\n'}
active?.<span className="fn">let</span> {'{'} <span className="fn">println</span>(<span className="str">"Connected to ${'{'}it.serverId{'}'}"</span>) {'}'}</code></pre>

        <PageNav 
          prev={{ href: '/sdk/docs/v3/server-discovery', title: 'Server Discovery' }}
          next={{ href: '/sdk/docs/v3/smart-connect', title: 'Smart Connect' }} 
        />
      </SDKDocsLayout>
    </>
  );
