  ...
  return (
    <>
      <SEO 
        ...
        canonical="/sdk/docs/v3/server-discovery"
      />
...
sdk.<span className="fn">getServers</span>(filter = <span className="type">ServerFilter</span>({'\n'}
    ...
    status   = <span className="str">"active"</span>{'\n'}
        ...
        <p>
          The SDK supports protocol filtering in two ways: (1) by passing a protocol ID string to <code>ServerFilter.protocol</code>
          (use values from <code>getRegistryConstants()</code> to stay in sync with your backend), or (2) using the typed convenience method
          <code>getServersByProtocol(protocol: VpnProtocol)</code>. <code>VpnProtocol.AMNEZIA</code> is fully supported — the server model
          includes feature flags for AmneziaWG, and the backend's registry constants response contains the appropriate protocol ID to use in filters.
        </p>
...
{'\n'}
<span className="cmt">// Clear cached data per domain</span>{'\n'}
sdk.<span className="fn">clearServerCache</span>(){'\n'}
sdk.<span className="fn">clearLocationCache</span>()</code></pre>

        <PageNav 
          prev={{ href: '/sdk/docs/v3/getting-started', title: 'Getting Started' }}
          next={{ href: '/sdk/docs/v3/connection-lifecycle', title: 'Connection Lifecycle' }} 
        />
      </SDKDocsLayout>
    </>
  );
