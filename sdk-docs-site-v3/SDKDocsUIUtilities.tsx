  ...
  return (
    <>
      ...
      <SDKDocsLayout title="UI Utilities">
        ...
        <h3>Correction — provider access from the public SDK API</h3>
        <p>
          The SDK source code in this repository contains provider classes (<code>ServerListProvider</code>, <code>LocationPickerProvider</code>, <code>QuickConnect</code>),
          but they currently use <code>internal constructor</code> and are not obtainable via a public factory method on <code>FyreWayClient</code>
          (for example, there is no public <code>sdk.getQuickConnectProvider()</code> method in the current SDK sources).
        </p>
        <p>
          <strong>Current SDK behavior:</strong> you can still build the same UI flows by calling the public APIs
          (<code>getServers()</code>, <code>getCountries()</code>, <code>getCities()</code>, <code>smartConnect()</code>) and then using the utility/extension functions below
          to convert servers into UI-ready models.
        </p>
        <p>
          <strong>Needs manual verification:</strong> whether this documentation is intended for a future SDK version where provider factories are exposed publicly.
        </p>
...
<span className="kw">val</span> results = locationPickerProvider.<span className="fn">searchLocations</span>(<span className="str">"New"</span>)</code></pre>
        ...
        <p>
          The following utility classes exist in the SDK under <code>com.fyreway.vpn.sdk.util</code>. They are safe to use directly from app code.
        </p>
