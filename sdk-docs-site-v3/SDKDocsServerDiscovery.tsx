...
sdk.<span className="fn">getServers</span>(filter = <span className="type">ServerFilter</span>({'\n'}
    ...
    status   = <span className="str">"active"</span>{'\n'}
        ...
        <h3>Updated note — protocol filtering and Amnezia</h3>
        <p>
          The SDK supports protocol filtering in two different ways:
          (1) by passing a protocol ID string to <code>ServerFilter.protocol</code> (which should come from <code>getRegistryConstants()</code>),
          and (2) by using the typed convenience method <code>getServersByProtocol(protocol: VpnProtocol)</code>.
        </p>
        <p>
          In the SDK source code in this repository, <code>VpnProtocol</code> includes <code>AMNEZIA</code> and the server model includes feature flags for AmneziaWG.
          If your backend publishes Amnezia protocol IDs via registry constants, you can drive a protocol picker dynamically and pass the chosen value in filters.
        </p>
        <p>
          <strong>Needs manual verification:</strong> the exact registry protocol ID(s) used for Amnezia (and whether they differ from server payload strings such as
          <code>"amneziawg-v20"</code>) should be validated against your backend constants response.
        </p>
...
sdk.<span className="fn">clearCache</span>()</code></pre>
        ...
        <h3>Correction — cache clearing methods in the current SDK</h3>
        <p>
          The SDK in this repository does not expose a single <code>clearCache()</code> method on the main client.
          Instead, caches are cleared per domain via <code>clearServerCache()</code> and <code>clearLocationCache()</code>.
        </p>
        <div className="sdk-code-header"><span>Kotlin — clear caches</span></div>
        <pre><code>sdk.<span className="fn">clearServerCache</span>(){'\n'}
sdk.<span className="fn">clearLocationCache</span>()</code></pre>
