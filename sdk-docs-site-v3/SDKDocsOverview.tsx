  ...
  return (
    <>
      ...
      <SDKDocsLayout title="Overview">
        ...
        <h2>Correction — Version information in this repo</h2>
        <p>
          This website currently displays <code>SDK v1.2.0</code> in several places. In the SDK source code included in this repository,
          the published Gradle module version is <code>1.1.1</code> (see <code>fyreway-vpn-sdk/build.gradle.kts</code>) and
          <code>FyreWaySDK.VERSION</code> is <code>"1.1.0-alpha"</code> (see <code>fyreway-vpn-sdk/src/main/kotlin/com/fyreway/vpn/sdk/FyreWaySDK.kt</code>).
        </p>
        <p>
          <strong>Needs manual verification:</strong> which exact version is currently published to your Maven repository and which version this website should target.
          Until that is confirmed, treat the version badges and "What's New" claims as potentially out of date.
        </p>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              <td><strong>Amnezia</strong></td>
              <td><code>.conf</code></td>
              <td>51820</td>
              <td><code>VpnProtocol.AMNEZIA</code></td>
            </tr>
          </tbody>
        </table>
        ...
        <h3>Current SDK behavior — Amnezia naming &amp; detection</h3>
        <p>
          The SDK supports <strong>Amnezia</strong> as a protocol constant and can parse multiple backend strings into it.
          In practice, backend payloads may include identifiers like <code>"amnezia"</code> or variants such as <code>"amneziawg-v20"</code>.
          The SDK handles this in protocol parsing and server capability checks.
        </p>
        <p>
          <strong>Needs manual verification:</strong> the exact protocol ID you must send in <code>CreateProfileRequest.protocol</code> when requesting an Amnezia profile
          depends on your backend contract. The SDK request model uses a <code>String?</code> for protocol and does not hardcode a single required value in the public API.
        </p>
        ...
        <h3>Correction — Java/Kotlin target used by the SDK module</h3>
        <p>
          The SDK module in this repository targets Java <strong>17</strong> (see <code>fyreway-vpn-sdk/build.gradle.kts</code> where
          <code>sourceCompatibility</code> / <code>targetCompatibility</code> and Kotlin <code>jvmTarget</code> are set to <code>17</code>).
          If your build is configured for Java 8 bytecode only, you may need to update toolchains accordingly.
        </p>
        ...
        <table>
          ...
          <tbody>
            <tr>
              <td><Link to="/sdk/docs/whats-new">What's New — April 2026</Link></td>
              <td>Full changelog and migration guide for SDK v1.2.0</td>
            </tr>
            <tr>
              <td><Link to="/sdk/docs/getting-started">Getting Started</Link></td>
              <td>Install, configure, activate your first device</td>
            </tr>
            <tr>
              <td><Link to="/sdk/docs/server-discovery">Server Discovery</Link></td>
              <td>Browse servers, filter by location/protocol, use caching</td>
            </tr>
            <tr>
              <td><Link to="/sdk/docs/connection-lifecycle">Connection Lifecycle</Link></td>
              <td>End-to-end connect → use → disconnect flow</td>
            </tr>
            <tr>
              <td><Link to="/sdk/docs/smart-connect">Smart Connect</Link></td>
              <td>One-call server selection with strategies</td>
            </tr>
            <tr>
              <td><Link to="/sdk/docs/error-handling">Error Handling</Link></td>
              <td>VpnResult pattern, exception types, retry logic</td>
            </tr>
            <tr>
              <td><Link to="/sdk/docs/ui-utilities">UI Utilities</Link></td>
              <td>Display models, providers, formatters for fast UI building</td>
            </tr>
            <tr>
              <td><Link to="/sdk/docs/configuration">Configuration Reference</Link></td>
