  ...
  return (
    <>
      <SEO 
        ...
        canonical="/sdk/docs/smart-connect"
      />
...
sdk.<span className="fn">smartConnectForCategory</span>(category = <span className="str">"p2p"</span>,      tier = <span className="str">"premium"</span>, country = <span className="str">"NL"</span>)</code></pre>
    ...
    <span className="kw">val</span> alternatives: <span className="type">List&lt;Server&gt;</span>           <span className="cmt">// Next-best options (from backend or empty on offline path)</span>{'\n'}
        ...
        <h3>Correction — SmartConnectResult fields in the current SDK</h3>
        <p>
          The <code>SmartConnectResult</code> model in the SDK source code included in this repository contains:
          <code>server</code>, <code>reason</code>, <code>score</code> (as a <code>Double</code>), and <code>alternatives</code>.
          It does not include <code>strategy</code> or <code>criteria</code> fields in the public model.
        </p>
        <p>
          If you need to display the selected strategy/criteria in your UI, keep those values in your own app state alongside the SDK result.
        </p>
...
<span className="fn">println</span>(<span className="str">"${'{'}display.flagEmoji{'}'} ${'{'}display.locationText{'}'}"</span>)  <span className="cmt">// "🇺🇸 New York, US"</span></code></pre>
        ...
        <h3>Correction — provider exposure in the current SDK</h3>
        <p>
          The SDK source code in this repository includes provider classes like <code>QuickConnect</code>, <code>ServerListProvider</code>, and <code>LocationPickerProvider</code>,
          but they currently have <code>internal constructor</code> and there is no public SDK method named <code>getQuickConnectProvider()</code> on <code>FyreWayClient</code>.
          That means integrators cannot currently obtain these providers through the public SDK API as shown above.
        </p>
        <p>
          <strong>Current SDK behavior:</strong> for one-tap experiences, use the existing Smart Connect convenience methods:
          <code>smartConnect()</code>, <code>smartConnectByLocation()</code>, <code>smartConnectForStreaming()</code>, <code>smartConnectForP2P()</code>, and <code>smartConnectForCategory()</code>.
        </p>
        <p>
          <strong>Needs manual verification:</strong> whether your docs site is intended to document a future SDK version where provider accessors are added,
          or whether provider classes should be treated as SDK-internal utilities.
        </p>

        <PageNav 
          prev={{ href: '/sdk/docs/connection-lifecycle', title: 'Connection Lifecycle' }}
          next={{ href: '/sdk/docs/error-handling', title: 'Error Handling' }} 
        />
      </SDKDocsLayout>
    </>
  );
